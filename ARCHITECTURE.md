...
8. **`pnpm typecheck` does not currently pass, so the type-level guarantees are unenforced.** The baseline is **137 errors on web** — 124 of them in `presentation/` (pages, hooks, components) — and **33 on mobile**. The shared packages are clean and gated at zero. Counts drift, so measure rather than trust this paragraph; `.github/quality-baseline.json` is the number CI actually enforces.
    ...
    **Most of the web errors had one cause, and it is now fixed.** `CollectionSpec.schema` is typed `unknown`, `CollectionRuntime` types the injected tanstack builders as `(config: never) => object`, `buildCollectionOptions` therefore returns a bare `object`, and `defineCollection` ended in `createCollection(... as any)`. Four layers of erasure, so every collection's row type was lost and every `useLiveQuery` handed the UI `unknown` fields — 76 of the original 137 errors said "`unknown` is not assignable".
    ...
    Both apps now use the typed form. What remains after that repair is a handful of router/expo-router typings, not row-shaped errors.

    **The domain types and the row schemas described the same rows, and disagreed.** `Task.opened_at` was a plain `Date`; the wire is `string | Date | undefined` (a `Date` off Electric, a `string` rehydrated from the local store, absent where the column is optional). The domain types lost: they were unenforced, while the row schemas are what zod is written against, and call sites were already working around them with `instanceof Date ? … : new Date(… as string)` and casts. Timestamps in `domain-types` are now `WireTimestamp`, read through `toDate()` from contracts, and components that took a domain type where a live query hands them a row now take the row (or a `Pick<>` of it). Several hand-rolled structural copies — `SyncedResource`, `SidebarTeam` — were replaced by slices of the canonical row.

    **One caveat that survives all of this.** Row schemas validate CLIENT MUTATIONS, not rows arriving from sync (see the parser note in each app's `collections/_shared.ts`). So for a jsonb column the row type is optimistic in the same way the domain type was: `z.preprocess(unwrapJsonb, z.enum(…))` advertises an unwrapped enum, while a synced row still holds the raw `'"High"'` string. `unwrapJsonb` at the call site is what makes it true, and that step is load-bearing rather than decorative — do not delete it because the type looks like it already happened.
