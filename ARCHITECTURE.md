...
**Still parallel and duplicated:** the collection *definitions* (the per-table Electric/persisted collections in `application/collections/*.ts`) and the offline-executor wiring. These stay per-app because they are the most platform-entangled — persistence engine, project-scoping, GC tiers. Their common *helpers*, though, are already shared: the framework-free parts of `collections/_shared.ts` — the shape retry, the memberships shape-error tracking (§6), the GC tier constants and `coerceBool` — live in `sync-core`, and each app's `_shared.ts` re-exports them and keeps only what genuinely differs (web's `origin`; mobile's `cookieFetch`, `safeCleanup`, and the Hermes timestamp `parser`). So the earlier drift risk has largely moved into shared code. (The `CONFLICT` code that was once in mobile's non-retriable set and missing from web's can no longer diverge: both derive it from `domain-types`.) The lazy-load rework (timestamp `seen_state` replacing per-item `reads`, the `inbox_mentions` / `my_tasks` badge slices, and idle-GC on the heavy collections) was ported to mobile to keep the two collection layers in step; both now run it.
...
5. **Client-logic duplication — MOSTLY RESOLVED.** The actions layer, the mutation-fns spine, the common collection helpers (`_shared.ts`), and the non-retriable-code set are now single-sourced (`@buildinlime/sync-core` and `@buildinlime/domain-types` / `@buildinlime/contracts`), which removes the largest drift surface — the kind that had already produced the `CONFLICT` divergence. Still duplicated: the per-table collection *definitions* and the offline-executor wiring (§10), the most platform-entangled pieces.
