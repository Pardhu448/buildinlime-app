...
**The row schemas are shared too**, which closed the real drift risk. Previously web derived its own from drizzle (`createSelectSchema`) while mobile hand-wrote the equivalent, with nothing tying them together; mobile's copy had already lost `properties.channel_id` and `properties.createdby_id` (latent only because no call site read them yet). The server now holds the contract to its tables: `database/schema/row-contract-assertions.ts` asserts every drizzle column is covered by the row schema, so a migration that adds a synced column fails the typecheck naming the column — subject to the caveat in §12.8.

**What those schemas do and do not do at runtime, because earlier revisions of this document got it backwards.** The clients validate **client mutations** against them, not rows arriving from sync: `validateData` lives in `@tanstack/db`'s `collection/mutations` and is called from the insert/update paths, while `collection/sync.js` — which writes what Electric delivers — never calls it. So:

- A **synced row is not validated**. A column missing from the schema is present at runtime but untyped, so call sites simply never use it — which is how mobile's copy lost two columns quietly.
- A **mutation is validated**, and zod strips unknown keys, so a missing column IS dropped from your own optimistic row until the server's version syncs over the top. That is a writer-only flicker, easy to dismiss as a rendering glitch.
- The schemas' `preprocess` steps (`coerceBool`, `unwrapJsonb`) are part of validation, so they do **not** run on sync either. Any coercion a synced row receives comes from Electric's own parser instead — a separate mechanism, described next.

**A row schema is therefore a description of the wire, not a runtime guarantee about it.** This is the single most load-bearing thing to know when reading row types: `t.completed` may be typed `boolean` and hold the string `"false"`, which is exactly why call sites carry `coerceBool` and `unwrapJsonb` rather than trusting the declared type. Those calls are load-bearing, not redundant.

The row schemas model the **wire**, not the Postgres types: a timestamp is `string | Date` (a `Date` fresh off Electric, a `string` once rehydrated from the local SQLite store), read with `toDate()`.

Coercion of synced values is Electric's job, not the schema's. `@electric-sql/client`'s `defaultParser` already covers `int2/int4/int8/bool/float4/float8/json/jsonb`, and a `parser` passed in the shape options is **merged over** it (`{ ...defaultParser, ...parser }`) rather than replacing it — so each app's `timestamptz`/`int8` overrides do not disable the rest. A parser only fires where the shape response carries column-type metadata, though, which is why raw `'"High"'` and the string `"false"` are both observed in practice and why the client-side `unwrapJsonb` / `coerceBool` calls stay. Treat them as the guarantee; treat the parser as an optimisation.
...
8. **`pnpm typecheck` passes on both apps — RESOLVED.** It did not for most of this project's life; the baseline was 137 errors on web and 33 on mobile, and `.github/quality-baseline.json` now pins **typecheck at 0 everywhere**, which makes it a hard CI gate rather than a debt number. Lint remains red (180 web / 1 mobile) and is still ratcheted.

    This matters beyond tidiness, because several guarantees this document claims are *enforced by typecheck* — the mobile `AppRouter` contract (§12.4), the row-contract assertions (§10) — and against a wall of pre-existing errors a new one is invisible. They are real now.

    **What the backlog turned out to be hiding.** Nearly all of it traced to one cause: row types were erased in four places (`CollectionSpec.schema` is `unknown`, `CollectionRuntime` types the injected tanstack builders as `(config: never) => object`, `buildCollectionOptions` returns a bare `object`, and `defineCollection` ended in `createCollection(... as any)`), so every `useLiveQuery` handed the UI `unknown` fields. Restoring the row types exposed genuine bugs that had been invisible for exactly as long: `bu.descritption` (a typo — build unit descriptions rendered as undefined), an `onBuildUnitClick` that declared a field the rows do not have, `<Link href={…}>` on routes that therefore pointed at the current page, two route literals with a trailing slash not in the generated union, and `new Date(undefined)` in the inbox/my-tasks sorts.

    Two traps are worth recording because both cost real time. `createCollection<T>`'s first type parameter is the **schema**, not the row (the row is `InferSchemaOutput<T>`), so passing a row type there yields a garbage row type such as `{}` rather than an error — measurements taken that way suggested the repair *added* 100+ errors when it in fact removed 80. And a large drop in the count can mean `tsc` bailed out on a parse error rather than progress; check for `TS1xxx` and for the error list collapsing to one or two files before believing it.

    **The ratchet is only as good as the committed numbers**, which failed twice: once left stale at 224/34 against an actual 137/33 (87 errors of slack a regression could hide in), and once when merging two PRs restored an older copy of the baseline file and handed mobile 30 back silently. An improvement now FAILS the ratchet until the number is committed, which is what keeps it monotonic.
