...
**4. Action-module resets are asymmetric, and web is missing one.** Mobile has an `actions/index.ts` barrel whose `resetAllOfflineActions()` covers every one of its five action modules. Web has no barrel: `signOutAndDispose` enumerates them by hand and calls five of six — `resetSeenActions` is exported and **never called anywhere**, so the seen action keeps its binding to the previous session's executor and collection across a sign-out. A barrel makes the enumeration structural instead of remembered, which is the actual fix; calling the sixth reset is the immediate one.
...
8. **`pnpm typecheck` does not currently pass, so the type-level guarantees are unenforced.** As of this writing the baseline is **224 errors on web** (concentrated in `presentation/` — pages, UI components, hooks — plus 11 in the tRPC routers and 3 in `tailwind.config.ts`) and **34 on mobile** (mostly `app/`). None are in the collections, contracts or sync-core layers, which are clean. This matters more than a tidiness backlog: several guarantees this document claims are *enforced by typecheck* — the mobile `AppRouter` contract (§12.4) and the row-contract assertions (§10) both fail the build on drift **only if someone runs a build that is expected to be green**. Against a 258-error baseline, a new error is invisible, and no CI gate can be switched on until the count reaches zero. Until then these guards catch drift only for a developer who diffs the error list by hand. Getting to a green baseline is what makes every other type-level claim here real.
