...
// Generated by scripts/generate-api-routes.mjs during `pnpm build`. See that file
// for why: TanStack falls through to the SSR path when no server route matches,
// and in SPA mode that throws — so an unknown /api/* path returned 500 instead of
// 404. Matching against the real route list keeps unmatched paths away from the
// handler entirely, rather than catching an internal error and guessing.
const apiRoutes = JSON.parse(
  readFileSync("./dist/server/api-routes.json", "utf8"),
).routes.map((r) => new RegExp(r.pattern))
...
function isServerRoute(pathname) {
  // /_server* is TanStack's server-function endpoint; it does its own dispatch.
  return pathname.startsWith("/api/") || pathname.startsWith("/_server")
}

/** True when the path is a declared API route (so the handler can serve it). */
function isKnownApiRoute(pathname) {
  return apiRoutes.some((re) => re.test(pathname))
}
...
const server = serve({
  ...
  fetch: (request) => {
    ...
    if (isServerRoute(pathname)) {
      // An /api path that is not a declared route is a 404, not a server fault.
      // Scanners probe /api/.env, /api/graphql and friends constantly; a 500
      // both fills the logs and signals to a prober that something is listening.
